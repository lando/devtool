...
class Plugin {
   ...
   */
  static async fetch(plugin, { dest = os.tmpdir(), installer = Plugin.installer, config = Plugin.config, type = 'app' } = {}) {
    ...
    const tmp = path.join(os.tmpdir(), generateId());
   ...
   * TBD
   */
  static async info(plugin, { config = Plugin.config } = {}) {
  ...
  static isValid(plugin) {
    ...
    if (isClass(plugin.constructor) && plugin.constructor.name === 'Plugin') {
      return (
        Object.keys(plugin.manifest).length > 0 ||
  ...
  constructor(
    ...
    this.manifest = require('../utils/normalize-manifest-paths')({ ...this.pjson.lando, ...this.#load(...loadOpts) }, this.root);
  ...
  async hasUpdate(channel = 'stable') {
    ...
    try {
      ...
      // better 404 message
      if (error.statusCode === 404) error.message = `Could not find a plugin called ${this.name}`;
      // throw
      throw makeError({ error });
    }
  }
